...
public:
    ...
    static int DrawFromBMPFile(const std::filesystem::path& filename, std::vector<uint8_t>& out)  {
            ...
            auto imgData = read_bitmap (filename);
            if(imgData == std::nullopt) {
                        ...
                        for (size_t h = 0; h < imgData.height; h+=8) {
                            ...
                            for (size_t bit = 0; bit < 8; bit++) {
                                ...
                                if(pixel_index < imgData.pixels.size()) {
                                    ...
                                    std::cout << pixel << " " << 7-w%8 << " " << bit << std::endl;
                                    if(pixel[7-w%8] ==  true) { 
                                        byte |= (1 << (7-bit));
                                    }
                                }
                            ...
                            std::cout << "komurka: " << std::dec << out_index << std::hex <<" byte: 0b" << std::setw(2) << static_cast<std::bitset<8>>(byte) << std::dec << " w: " << w << " h:" << h <<  std::endl;
            ...
            static std::optional<imageData> read_bitmap(const std::filesystem::path& filename) {
            ...
            if(!file) {
                ...
                file.close();
                return imageData{static_cast<size_t>( bmpInfo.biSize), static_cast<size_t>(bmpInfo.biWidth), static_cast<size_t>(bmpInfo.biHeight), pixels};
            }
};